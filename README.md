...
| **演员资料** | 抓演员本人的简介 / 出生日期 / 出生年份 / 出生地 / 外部 ID（AVデータバンク + AV-League + Wikipedia 三源合并），并列出**该演员在媒体库里的作品**；默认**只填空白**，单卡可**勾选覆盖已有值**，可一键回滚 |
...
4. **演员头像** —— 先选媒体库（默认全部），默认只列出无头像的演员，点「刮削」单个处理，「批量刮削头像」按数量批量跑。点「资料」可以抓演员本人的资料，并把**该演员在媒体库里的作品**一并列出来。
...
> 当前对齐的是 **v1.2.0**：Release 与 main 的 md5 同为 `63e4156940d9ff9de942d1e394191f1e`。
...
只读模式验：资料源清单与写入策略（`only_blank` / 单卡 `as_picked`）、挑一个真实演员**逐字段**核对「只填空白」、
...
镜像标签由 tag 推导：`v1.2.0` → `1.2.0` / `1.2` / `1` / `latest`（`latest` 始终跟着最新正式版）。
...
| `smoke_profile.py` | 演员资料只读冒烟（源清单 / 只填空白 / 预览无副作用 / 错误路径）；`PROFILE_LIVE=1` 再加真实写入 → 回滚 → 校验还原，**以及勾选覆盖已有值 → 回滚 → 校验还原** | 起 exe + 真实 config |
| `verify_profile_view.py` | 演员资料面板**渲染**（源勾选、头像按钮文案、对照表可勾选/默认勾选状态、媒体库作品区块、勾选覆盖的按钮文案与配色、作品「再加载」分页、同步历史状态；样本非空） | 起 exe + 无头 Edge |
